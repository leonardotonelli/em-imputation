...
def initialize_parameters(data_obs):
    """
...
def generate_data_with_nan(num_samples, dimensions, missing_rate=0.2):
    """
    Generate data from multivariate Gaussian and introduce missing values (NaN).
    """
    mu_real = np.arange(dimensions) * 5
    Sigma_real = np.diag(np.arange(1, dimensions + 1)) + 0.5
    
    data_complete = np.random.multivariate_normal(mu_real, Sigma_real, num_samples)
    
    data_obs = data_complete.copy()
    mask = np.random.rand(*data_obs.shape) < missing_rate
    data_obs[mask] = np.nan
    
    return data_obs, mu_real, Sigma_real
...
data_obs = np.array(pd.read_csv("synthetic_multivariate\\MAR_missing_30pct.csv", skiprows=0))

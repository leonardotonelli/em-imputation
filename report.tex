...
All simulations and applications, including the data generation processes for MVN and GMM, the implementation of the EM algorithm, and the evaluation scripts, are available in the accompanying code repository \cite{repository}. The implementation relies on standard Python libraries (\texttt{numpy}, \texttt{scikit-learn}, \texttt{scipy}) to ensure reproducibility and facilitate further research.
...
The selection of the hyperparameter $k$ was tailored to the specific demands of each simulation 
scenario. For the Multivariate Normal (MVN) experiments, 
where the focus was on high-dimensional parameter recovery across a vast experimental grid, 
we set $k=10$. While this is a heuristic, it represents a standard practice in many practical 
applications, striking an optimal balance between bias-reduction and the management of 
computational complexity in large-scale simulations. In contrast, for the Semi-Supervised 
GMM scenarios, the lower dimensionality of the imputation task (univariate label imputation) 
allowed for a more rigorous approach. In this setting, we implemented a 10-fold cross-validation
 for each individual dataset to select the optimal number of neighbors. This ensured that the 
 choice of $k$ was dynamically tuned to maximize classification performance, leveraging the 
 lighter computational burden of the semi-supervised framework to achieve higher predictive 
 precision.
...
\end{enumerate}
The imputation was implemented using the \textit{miceforest} library \cite{miceforest2025}. Aside 
from default settings, we capped the number of iterations at five. This choice was informed 
by computational efficiency and is supported by existing literature suggesting that five 
iterations are often sufficient 
for the MICE algorithm to achieve convergence and stable performance \cite{vanbuuren2011mice}.
...
\paragraph{Parameters of Random Forest} In our study, we decided to use the default parameters. This choice was made due to the fact that the main focus of the project wasn't searching for the optimal parameter to train the best classifier. 
Some key default hyperparameters are: \texttt{n\_estimators=100}, \texttt{criterion='gini'}, no maximum depth. 
...
Unlike the synthetic amputation described in Section 3.2, this dataset exhibits a 
``natural'' missingness mechanism derived from clinical disagreement. Ground truth 
labels were established only when there was unanimous agreement among 7 endoscopists 
(4 experts, 3 beginners). Cases with disagreement among experts were treated as having 
missing labels (NA). This mechanism effectively simulates a \textit{Missing Not at Random} 
(MNAR) or complex MAR scenario, where the missingness is intrinsic to the diagnostic 
difficulty of the lesion \citep{Lyu2023}. This procedure results in a 42\% of missing labels.
...
\begin{itemize}
    \item \textbf{High Dimensionality ($p=10$):} A setting where the sample-to-feature ratio is low ($N/p \approx 7.6$), challenging the covariance estimation. In this setting the explained variance is 73\%.
    \item \textbf{Low Dimensionality ($p=3$):} A compressed setting ($N/p \approx 25$) designed to satisfy the asymptotic requirements of the EM algorithm.In this setting the explained variance is 73\%.
\end{itemize}
% --- Section 4: Results ---
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/synthetic_multivariate/error_heatmap_all.png}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.6\textwidth]{plots/additional_visualizations/mvn_covariance_structures/MNAR_Strong_cov_error.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{plots/synthetic_multivariate/MNAR/sample_size_error_MNAR.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{plots/synthetic_multivariate/MNAR/sample_size_time_MNAR.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/synthetic_gmm/error_heatmap_all_gmm.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{plots/additional_visualizations/gmm_clustering_scenarios/GMM_Time.png}
...
Implementing a semi-supervised Gaussian Mixture Model (GMM) requires assuming that the underlying data follows a Gaussian distribution. In practice, these strict distributional 
assumptions are rarely met by empirical data and, consequently, are often overlooked by 
practitioners. Our preliminary diagnostic tests (provided in the Appendix) confirmed that 
the dataset does not strictly adhere to these assumptions.

However, GMMs have historically demonstrated good robustness and  
performance even when these theoretical conditions are not perfectly satisfied (\cite{Setiadi2024, sampaio2024regularizationoptimizationmodelbasedclustering, Liu2022GMMchi}). 
This empirical success motivated us to proceed with the analysis, testing the 
framework’s resilience and effectiveness in a realistic clinical context.
...
\end{figure}
Other than these, additional results' plots can be founded in the code repository \cite{repository}.


\subsection{Normality Diagnostics for Real-World Data}

To evaluate the adherence of the Gastrointestinal Lesions dataset to the Gaussian assumptions 
required by the semi-supervised EM algorithm, we performed a series of multivariate normality 
diagnostics for each class. Specifically, we generated Q-Q plots of the squared 
\textbf{Mahalanobis distances} against a $\chi^2$ distribution (Figure \ref{fig:grid_2x2}) 
and conducted the \textbf{Henze-Zirkler (HZ) test} for multinormality. 

These diagnostics were performed using the full set of ground-truth labels; conducting these 
tests on the observed labels alone was not ideal, as the resulting sample sizes for 
each cluster would have been too small to provide sufficient statistical power. The HZ tests 
rejected the null hypothesis of multinormality in nearly all experimental configurations, with 
the sole exception of Cluster 0 in the 2-Principal Component case. 

However, even in the instance where the test failed to reject the null hypothesis, 
the corresponding Q-Q plots exhibit noticeable non-linearities and deviations 
from the reference line. This suggests that the formal test results may not be 
entirely reliable in this context due to the small sample size ($N=76$), which often 
lacks the power to detect departures from normality or, conversely, may produce unstable 
results. The lack of a clear linear trend in the Q-Q plots across all dimensions indicates 
that the Gaussian assumption is likely violated, yet the EM algorithm's performance suggests 
it retains a degree of practical robustness to these distributional deviations.
...
\begin{figure}[htbp]
     ...
     \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=\textwidth]{plots/additional_visualizations/real_example/qq_10d_0.png}
         \caption{Class 0, 10 Principal Components}
     \end{subfigure}
     \hspace{5pt}
     \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=\textwidth]{plots/additional_visualizations/real_example/qq_10d_1.png}
         \caption{Class 1, 10 Principal Components}
     \end{subfigure}
     ...
     \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=\textwidth]{plots/additional_visualizations/real_example/qq3d0.png}
         \caption{Class 0, 2 Principal Components}
     \end{subfigure}
     \hspace{5pt}
     \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=\textwidth]{plots/additional_visualizations/real_example/qq3d1.png}
         \caption{Class 1, 2 Principal Components}
     \end{subfigure}
     
     \caption{QQ plots of Mahalanobis distance}
     \label{fig:grid_2x2}
\end{figure}

...
While the EM algorithm is mathematically elegant and guarantees monotonic convergence of the likelihood, it is not without limitations. It can be sensitive to initialization, risking convergence to local optima rather than the global maximum. Furthermore, the standard EM algorithm relies heavily on specific distributional assumptions (typically Gaussian) regarding the data generation process. This project is motivated by these limitations: we aim to understand how robust the EM algorithm is when these assumptions are stressed by varying degrees and patterns of missingness.
...
\begin{enumerate}
    \item \textbf{Fisher's Iris Dataset:} A classic dataset with established multivariate properties, useful for validating our initial Gaussian assumptions.
    \item \textbf{Wine Quality Dataset (UCI):} A higher-dimensional dataset where variables may exhibit complex correlations, testing the algorithm's scalability.
    \item \textbf{Boston Housing Dataset:} To test performance on data that may contain heavier tails or outliers.
\end{enumerate}
